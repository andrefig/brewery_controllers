...
class ActorAPI(object):
    ...
    #def switch_actor_exib_on(self, id, power=None):
    #    actor = self.cache.get("actors").get(id)
    #    if actor.state == 1:
    #        return 1
    #    actor.instance.on(power=power,so_exibe=True)
    #    actor.state = 1
    #    #if power is not None:
    #    #    actor.power = power
    #    self.emit("SWITCH_ACTOR", actor)
    ...
    def switch_actor_exib(self, id):
        actor = self.cache.get("actors").get(id)
        try:
            estado = actor.instance.verif_status()
        except Exception as e:
            return
        #print("!")
        #print(estado)
        if estado == 0:
            actor.instance.off(so_exibe=True)
            actor.state = 0
            self.emit("SWITCH_ACTOR", actor)
        else:    
            actor.instance.on(so_exibe=True)
            actor.state = 1
            self.emit("SWITCH_ACTOR", actor)

    def switch_actor_read_status(self,id):
        actor = self.cache.get("actors").get(id)
        try:
            estado = actor.instance.verif_status()
        except Exception as e:
